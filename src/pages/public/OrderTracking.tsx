  ...
  return (
    <div className="min-h-screen bg-neutral-50 dark:bg-neutral-950">
      ...
      <main className="mx-auto max-w-lg px-4 py-8">
        <h1 className="text-xl font-bold text-neutral-900 dark:text-neutral-100">{t('orderTracking.thankYou')}</h1>
        <p className="mb-1 text-sm text-neutral-500 dark:text-neutral-400">
          {t('orderTracking.orderNumber', { id: order.id.slice(0, 8).toUpperCase() })}
        </p>
        <p className="mb-6 text-xs text-neutral-400 dark:text-neutral-500">{t('orderTracking.liveUpdates')}</p>
        ...
        <Card>
          <CardBody>
            ...
            <div className="flex items-center justify-between border-t border-neutral-100 pt-3 text-sm dark:border-neutral-800">
              <span className="text-neutral-500 dark:text-neutral-400">{t('orders.total')}</span>
            ...
            {delivery?.driver_name && (
              <div className="mt-4 rounded-lg bg-neutral-50 p-3 dark:bg-neutral-800/50">
                <p className="text-xs font-semibold uppercase text-neutral-500 dark:text-neutral-400">{t('orderTracking.driverInfo')}</p>
                <p className="text-sm font-medium text-neutral-900 dark:text-neutral-100">{delivery.driver_name}</p>
                {delivery.driver_phone && <p className="text-xs text-neutral-500 dark:text-neutral-400">{delivery.driver_phone}</p>}
              </div>
            )}
          </CardBody>
        </Card>
        ...
        {slug && (
          <Link
            to={`/r/${slug}`}
            className="mt-6 inline-block text-sm font-semibold text-brand-700 hover:underline dark:text-brand-400"
          >
